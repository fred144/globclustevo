...
def clump_filters(ds): 
    ...
    def clump2(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 19.99834153 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump3(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 57.31993441 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump4(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 57.41739477 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump5(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 57.52136552  #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump6(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 57.80006233 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump7(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 58.80562044 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump8(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 58.83392123 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump9(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 58.84616938 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump10(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 58.96074425 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump11(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 59.34228001 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump12(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 71.76056966 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
    ...
    def clump13(pfilter, data):
        ...
        star_age_myr = code_ages*H0inv_yr/1e6
        relative_ages = star_age_myr - star_age_myr.min()
        wanted_age = 73.59632006 #0 corresponds to the oldest star clump
        filter = np.logical_and(np.isclose(relative_ages, wanted_age), relative_ages >= 0)
        return filter
