...
for loop_num, output_num in enumerate(range(start_step, end_step + 1)):
    ...
    infofile = os.path.abspath(
        datadir + "/output_%05d/info_%05d.txt" % (output_num, output_num)
    )
    ...
    p.annotate_text(
        (0.73, 0.92),
        "SFC: {}".format(len(ad["SFC", "particle_birth_epoch"])),
        coord_system="figure",
    )
    p.annotate_marker(
        (0.71, 0.93),
        marker="x",
        coord_system="figure",
        plot_args={"color": "b", "s": 40},
    )

    p.annotate_text(
        (0.73, 0.89),
        "PSC: {}".format(len(ad["PSC", "particle_birth_epoch"])),
        coord_system="figure",
    )
    p.annotate_marker(
        (0.71, 0.90),
