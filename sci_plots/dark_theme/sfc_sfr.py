...
with plt.rc_context(
    ...
    with plt.style.context("dark_background"):
        ...
        bin_width_myr = 1
        fs035_interp_points = np.arange(
            t_myr_fs035.min(), t_myr_fs035.max(), bin_width_myr
        )
        ...
        fs070_interp_points = np.arange(
            t_myr_fs070.min(), t_myr_fs070.max(), bin_width_myr
        )
        ...
        sfr_fs035 = np.gradient(fs035_interp(fs035_interp_points)) / (
            bin_width_myr * 1e6
        )
        ax[1].plot(fs035_interp_points, sfr_fs035, color=fs35_color, lw=1.5, alpha=0.8)

        sfr_fs07 = np.gradient(fs070_interp(fs070_interp_points)) / (
            bin_width_myr * 1e6
        )
        ...
        plt.savefig(
            "../../../g_drive/Research/AstrophysicsSimulation/sci_plots/final/darkmode/sfc_sfr.png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.05,
        )
