    ...
    with plt.rc_context(
        ...
        for i, (f7_ds, f3_ds) in enumerate(zip(fs070_matched, fs035_matched)):
            ...
            try:
                ...
                f7_mask = (f7_masses_per_snapshot > star_mass_min) & (
                    f7_alpha < max_alpha
                )
            ...
            try:  # if there is only once cluster
                ...
                f3_mask = (f3_masses_per_snapshot > star_mass_min) & (
                    f3_alpha < max_alpha
                )
            ...
            f7_weights = np.where(f7_vir_mass > 700, 0.01, 100)
            f3_weights = np.where(f3_vir_mass > 700, 0.01, 100)
            ...
            f7_mc_theory_x = np.log10(
                np.geomspace(mc_f7_mass.min(), mc_f7_mass.max(), 100)
            )
            ...
            f3_mc_theory_x = np.log10(
                np.geomspace(mc_f3_mass.min(), mc_f3_mass.max(), 100)
            )
            ...
            with plt.rc_context(
                ...
                textstr_f7 = (r"$\mathrm{{t}} = {:.0f} \: \mathrm{{Myr}}$").format(
                    f7_t_myr
                )
