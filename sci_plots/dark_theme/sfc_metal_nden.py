...
with plt.rc_context(
    ...
    with plt.style.context("dark_background"):
        ...
        cb2.set_label(
            label="$\mathrm{z}_{\mathrm{formation}}$", fontsize=16, labelpad=6
        )
        ...
        f70 = mlines.Line2D(
            [], [], color="w", marker="o", ls="", label=r"$f_{*} = 0.70$"
        )
        f35 = mlines.Line2D(
            [], [], color="w", marker="P", ls="", label=r"$f_{*} = 0.35$"
        )
        ax.legend(
            # title="$\mathrm{SFE} \: (f_{*})$",
            loc="upper right",
        ...
        plt.savefig(
            "../../../g_drive/Research/AstrophysicsSimulation/sci_plots/final/darkmode/sfc_metal_nden.png",
            dpi=300,
            bbox_inches="tight",
            pad_inches=0.05,
        )
