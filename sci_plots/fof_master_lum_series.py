...
def consecutive_filter(x, y, percent_thresh):
    """If, in a 1d array, the y value changes by percent threshold
    with regards to previous value, drop that value.
    Thresh is the lower bound, anything above it will be included
    """
    consecutive_diff = np.diff(y) / y[:-1] * 100
    if type(percent_thresh) is tuple:
        # mask what to keep
        # ranges to keep, comes in tuples (-10,-1)
        # this means discard anything witin -10% to -1% difference
        # trims high freqeuncy and low magnitude differnces

        mask = (consecutive_diff < percent_thresh[0]) | (
            consecutive_diff > percent_thresh[1]
        )
...
# threshold_list = [-50, -40, -30, -20, -20, -20]
threshold_list = [
    -80,
    -60,
    -60,
...
for thresh in threshold_list:
    f7_t_sim_myr, f7_lum_in_gc, ma = consecutive_filter(
        f7_t_sim_myr, f7_lum_in_gc, thresh
    )
    f7_redshift = f7_redshift[ma]
    f7_total_mass = f7_total_mass[ma]
    f7_total_lum = f7_total_lum[ma]
    f7_mass_in_gc = f7_mass_in_gc[ma]
...
for thresh in threshold_list:
    f3_t_sim_myr, f3_lum_in_gc, ma = consecutive_filter(
        f3_t_sim_myr, f3_lum_in_gc, thresh
    )
    f3_redshift = f3_redshift[ma]
    f3_total_mass = f3_total_mass[ma]
    f3_total_lum = f3_total_lum[ma]
    f3_mass_in_gc = f3_mass_in_gc[ma]
...
with plt.rc_context(
    ...
    ax[0, 0].plot(
        f7_t_sim_myr, f7_lum_in_gc, label=r"In GCs, profiled", linewidth=4, c=cmap[1]
    )
    # ax[0, 0].scatter(f7_t_sim_myr, f7_total_lum, label=r"Total", s =3, c=cmap[0])
    # ax[0, 0].scatter(
    #     f7_t_sim_myr, f7_lum_in_gc, label=r"In GCs, profiled",s =3, c=cmap[1]
    # )
    ...
    ax[0, 1].plot(
        f3_t_sim_myr, f3_lum_in_gc, label=r"In GCs,  profiled", color=cmap[3], lw=4
    )
    ax[0, 1].legend(
        title="Pop II Stars",
        loc="lower right",
