...
DIR=/lustre/$USER/ramses/dwarf/data/cluster_evolution/fs035_ms10 #output directory
...
#output interval in Myr, defualt 5.0, changed to 2.5 after output 173 (test rerun)
#2.5 myr per output after 156 for refine i think
...
sed -i -e "s@^ngridtot.*@ngridtot=50000000@g" $NMLFILE
...
#######################################################
#      Set f_star = M_star/M_cloud to 0.35             #
#######################################################
sed -i -e "s@^SFC_f_star.*@SFC_f_star=3.5d-1@g" $NMLFILE #assume f_star = 0.35
#######################################################
